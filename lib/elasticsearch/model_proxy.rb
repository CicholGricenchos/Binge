class Elasticsearch::ModelProxy
  ...
  def reindex_scope(scope, batch_size: 500, keep_old: false, recreate: true)
    ...
    scope.find_in_batches(batch_size: batch_size) do |records|
      ...
      client.bulk body: (to_index.map do |record|
        {
          index: {_index: index_name, _type: @type_name, _id: record.id, data: record.search_data}
        }
      end)
    end
  ...
  def create_index
    ...
    client.indices.create index: index_name, body: @model.index_data
    index_name
  end
  ...
  def reindex_record record
    client.index index: @alias_name, type: @type_name, id: record.id, body: record.search_data
  end
